...
> `value` is a [MocQ config object](/docs/api)
  - which includes functions you define that define how to generate, connect to other keys in the configuration, and how to handle data when execute is called
...
const customConfigObj = {
  myCustomKey: {
    ...
    connections?: // ability to hook into resolved data from other keys in config & ammend myCustomKey data
...
Use connections to ammend generated data with data generated via another key in the top level configuration
...
ensures dependent data is there at time of execution allow data to be stiched together on the fly
