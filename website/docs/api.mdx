...
type to explicitly type your configuration
> without an explicit type `[mocq]` will attempt to infer the key type from the return of the generator function

<Tabs>
  <TabItem value="type" label="Create Custom Type" default>
    <CodeBlock language="ts"
      title="/index.ts"
      showLineNumbers>
...
<CodeBlock language="ts">
  {`type DataGenerator<T> = (index: number) => T`}
</CodeBlock>
...
<CodeBlock language="ts">
{`type DataConnection<T> = (connectionKeyData: X[], index: number, indexData: T) => Partial<T>`}
</CodeBlock>
