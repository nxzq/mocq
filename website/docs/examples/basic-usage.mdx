...
We don't really care how our data is connected just that it is in some way, a great time for randomness
...
export const getRandomArrElements = (data, maxCount) => {
  ...
  <TabItem value="ts" label="TypeScript">
    <CodeBlock
      ...
      title="/utils.ts"
      showLineNumbers>
{`export function getRandomArrElement(data: any[]) {
  return data[Math.floor(Math.random() * data.length)]
};

export function getRandomArrElements<T extends any>(data: T[], maxCount: number): T[] {
...
const { generate } = mocq({
  ...
  tags: {
    generator: generateMockTag,
      count: 25,
      connections: {
        users: (index, users)=>({ created_by: getRandomArrElement(users).alias }),
      }
  },
  elements: {
    generator: generateMockElement,
      count: 100,
      connections: {
        users: (index: number, users)=>({ created_by: getRandomArrElement(users).alias }),
        tags: (index: number, tags)=>({ tags: getRandomArrElements(tags, 2).map(x => x.id) }),
      }
  },
});
